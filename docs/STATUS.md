...
### 补充：整体黑白配色（2026-09-26，用户反馈，PR #9）

- **全局**：`--accent` 与 Element Plus 主色族由蓝紫改为黑白灰（`html.dark` 覆盖段）；primary 按钮文字转深色保证浅底可读；输入框聚焦边、流式光标随之变白
- **图标**：侧栏 logo / 用户头像 / AI 头像 / 登录页 logo 由蓝紫渐变改为黑底白字 + 细灰边框
- **新建面试**：去高亮，主色按钮改为中性灰底描边（hover 微亮）
- **会话列表**：时间从第二行移到标题行右侧（flex 布局，标题溢出省略号、时间不挤压），第二行仅留状态
- **彩色残留清理**：进行中状态绿→白、简历 chip 绿→灰、结束面试按钮红→中性 plain、登录页蓝色光晕→白色微光
- **验证**：浏览器截图两连（侧栏 + 聊天区）确认无蓝/紫/绿残留；DOM 计算样式核对 logo 背景、按钮配色、时间位置（`timeAfterTitle=true`）
...
### 补充：已结束会话可继续（2026-09-26 第二条用户反馈）

原设计"结束后不能再发言"（前端禁输入 + 后端 400），用户实测历史会话只能新开、无法继续。改为**发消息即自动续场**：

- **后端**：`append()` 解除 FINISHED 拒收（persistMessage 本就把状态翻回 IN_PROGRESS）；Agent 对 `agentState=DONE` 走 `RESUME_PLAN`——跳过决策、欢迎回来、**question/probe 计数清零开新一轮**、从简历/JD 未考察主题继续出题
- **前端**：已结束会话解除输入禁用，占位符"本场已结束——发送消息将继续练习"；done 后状态标签自动翻转（已结束 → 提问）
- **验证**：finish → 发消息（原 400 现成功）→ 回复"欢迎回来 + Redis Cluster 分片题"、DB 计数归零、状态 QUESTIONING/IN_PROGRESS；浏览器全流程（结束→输入→发送→标签翻转）通过；探针确认浏览器轮 83 think 事件 + 面板正常（个别轮次模型无思维链时不渲染面板，属设计行为）
